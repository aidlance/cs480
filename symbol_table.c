...
*
*   RETURNS:
*       Returns the token class if the token is
*       found in the symbol table. If it isn't
*       found, then this returns TOK_NO_CLASS
*
**************************************************/
...
{
    void *data = get( __id_table, str );
    if( NULL == data )
    {
        return( TOK_NO_CLASS );
    }

    return( ( (struct token_type *)data )->token_class );
...
{
    void *data = get( __keyword_table, str );
    if( NULL == data )
    {
        return( TOK_NO_CLASS );
    }
